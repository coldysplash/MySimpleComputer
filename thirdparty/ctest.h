...
#define ASSERT_STR(exp, real) assert_str (exp, real, __FILE__, __LINE__)
  ...
  void assert_wstr (const wchar_t *exp, const wchar_t *real,
...
#define ASSERT_DATA(exp, expsize, real, realsize)                             \
  ...
  void assert_equal (intmax_t exp, intmax_t real, const char *caller,
                     int line);
...
#define ASSERT_FAIL() assert_fail (__FILE__, __LINE__)
  ...
  void assert_dbl_near (double exp, double real, double tol,
                        const char *caller, int line);
...
#define ANSI_NORMAL "\033[0m"
  ...
  void
  assert_wstr (const wchar_t *exp, const wchar_t *real, const char *caller,
               int line)
  {
    if ((exp == NULL && real != NULL) || (exp != NULL && real == NULL)
        || (exp && real && wcscmp (exp, real) != 0))
      {
        CTEST_ERR ("%s:%d  expected '%ls', got '%ls'", caller, line, exp,
                   real);
      }
  }
  ...
  void
  assert_equal (intmax_t exp, intmax_t real, const char *caller, int line)
  {
    if (exp != real)
      {
  ...
  void
  assert_not_equal (intmax_t exp, intmax_t real, const char *caller, int line)
  {
    if ((exp) == (real))
      {
  ...
  void
  assert_not_equal_u (uintmax_t exp, uintmax_t real, const char *caller,
                      int line)
  {
    if ((exp) == (real))
      {
  ...
  void
  assert_dbl_near (double exp, double real, double tol, const char *caller,
                   int line)
  {
  ...
  void
  assert_dbl_far (double exp, double real, double tol, const char *caller,
                  int line)
  {
    double diff = exp - real;
    double absdiff = diff;
